...
class UsuarioController extends Controller
{
    ...
    */    
    ...
    public function newAction(Request $request)
    {
        ...
        if ($form->isSubmitted()) {
            try{
                ...
                $editarInformacion = $request->get('editarInformacion');
                if (! is_null($editarInformacion)) {

                    $usuario->setEditarInformacion(1);

                } else {

                    $usuario->setEditarInformacion(0);
    ...
    public function updateEstadoAction (Request $request) {
        ...
        $entity = $em->getRepository('CpdgAdministradorBundle:Usuario')->findOneById(array('id' => $idRegistro));
        $entity->setEditarInformacion($estado);
